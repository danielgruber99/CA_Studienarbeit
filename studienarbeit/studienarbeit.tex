...
Des Weiteren gibt es eine Compiler basierte Lösung, nämlich beispielsweise StackGuard, indem ein Sicherheitsmechanismus im Compiler eingebaut ist, der korrupte bzw. modifizierte Rücksprungadressen überprüft und damit zumindest den Sprung dorthin verhindert.
...
dann wieder zurück geschrieben wird.
...
mit dem Prozessor verbunden ist. Sollten also in einem Programm Schwachstellen ausgenutzt werden, können die Watchdogs aufgrund nicht zurückgesetzten Timer, den Prozessor neustarten und damit die Ausnutzung vorerst verhindern.

Zusammengefasst gibt es eine umfassende Menge von Schutzmaßnahmen, von denen hier einige dargestellt wurden. Anzumerken ist jedoch, dass diese Schutzmaßnahmen oft verhindern, dass schadhafter Code
ausgeführt wird, aber nicht, dass das System zum Absturz gebracht wird. Damit kann also die Verfügbarkeit des Systems beeinträcht werden, der in der englischen Literatur auch als Denial of Service (DOS) bekannt ist.
...
Mit \textit{\%n} kann insbesondere \textit{nprintf()} so ausgenutzt werden, dass ein Wert geschrieben wird an die Speicheradresse, die am Stack referenziert wird.
Der Angreifer kann also eine beliebige Adresse eingeben, dementsprechend \textit{\%n} ausnutzen, um dan diese eingegebene Adresse einen Wert zu schreiben.
