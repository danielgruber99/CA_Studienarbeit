...
Vorausgesetzt eine Speichervegleichsfunktion ist so implementiert, dass beim ersten nicht übereinstimmendem verglichenen Zeichen von der Funktion \textit{false} zurückgegeben wird,
...
Denn die Methode für das Überpüfen der PIN gibt beim ersten nicht korrekten Zeichen 'false' zurück, weshalb die die Ausführungsdauer der
...
Das erste Argument einer printf Funktion ist der sogenannte Format String, der mit den angegebenen Parametern beginnend mit \%, wie \%s, \%d, \%x usw., besimmt, wie nachfolgende Parameter
...
In der zweiten Eingabe, der Eingabe für den Benutzernamen, kann nun mit entsprechend vielen \textit{\%d} gefolgt von einem \textit{\%s}, welches sich genau an der Position des
vorher eingegebenen Alters, diese Schwachstelle ausgenutzt werden. Beispielsweise kann die Eingabe wie folgt aussehen: \textit{\%d\%d\%s}.
...
Dabei wird ein \textit{Stack Canary} - ein zufälliger bekannter Wert - im Stack neben der Rücksprungadresse geschrieben, während eine Kopie davon in einem General Purpose Register r0-r12 (siehe \autoref{fig: ARM Register Set})
...
Die Funktionalität der Infotainmentsysteme steigt zunehmend an, von Nutzung eines Navigationssystem über die Verbindung mit Mobilgeräten und Nutzung von weiteren Applikationen.
...
% DATASHEETS
\bibitem{MPU}  ST: \textit{Managing memory protection unit in STM32 MCUs}. n.d.. Zuletzt aufgerufen am: 09.07.2022. [Online]. https://www.st.com/resource/en/application\_note/dm00272912-managing-memory-protection-unit-in-stm32-mcus-stmicroelectronics.pdf
...
\bibitem{STM32F103Cx DataSheets}  ST: \textit{STM32F103Cx8/B DataSheets}. n.d.. Zuletzt aufgerufen am: 09.07.2022. [Online]. \\https://www.st.com/content/ccc/resource/technical/document/datasheet /33/d4/6f/1d/df/0b/4c/6d/CD00161566.pdf/files/CD00161566.pdf/jcr: content/translations/en.CD00161566.pdf
